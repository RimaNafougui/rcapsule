/**
 * Bulk background removal for all GlobalProduct catalog images.
 *
 * Usage:
 *   pnpm tsx scripts/remove-bg-catalog.ts
 *
...
async function removeBackground(base64Image: string): Promise<string> {
  ...
  const command = new InvokeCommand({
    ...
    Payload: JSON.stringify({ body: JSON.stringify({ imageBase64: rawBase64 }) }),
  });
  ...
  if (!body.image) throw new Error(`Lambda returned no image. Body: ${JSON.stringify(body)}`);
...
async function main() {
  console.log("Fetching unprocessed catalog products...");
