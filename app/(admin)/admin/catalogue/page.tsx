...
export default function AdminCataloguePage() {
  ...
  const { data, isLoading, mutate } = useSWR(
    `/api/admin/catalogue?${params}`,
    fetcher,
  );

  // Fetch metadata for filter options
  const { data: _meta } = useSWR(
    "/api/admin/catalogue?limit=1&offset=0&meta=true",
    fetcher,
  );
  ...
  const { data: allData } = useSWR(
    "/api/admin/catalogue?limit=2000&offset=0",
    fetcher,
  );
  ...
  async function handleConfirmDelete() {
    ...
    await fetch(`/api/admin/catalogue/${deleteTarget.id}`, {
      method: "DELETE",
    });
    onDeleteClose();
                    ...
                    <div className="relative aspect-[3/4] bg-content2">
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                        <Button
                          as={NextLink}
                          className="text-xs"
                          href={`/admin/catalogue/${p.id}/edit`}
                          radius="none"
                          size="sm"
                          variant="solid"
                        >
