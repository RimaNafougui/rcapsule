...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Explore ${itemCount} ${brandName} items catalogued on Rcapsule. Discover ${brandName} outfits, add pieces to your closet, and track cost-per-wear.`,
    openGraph: {
      title: `${brandName} Clothes & Outfits | Rcapsule`,
      description: `Browse ${itemCount} ${brandName} items on Rcapsule.`,
      url: `${baseUrl}/catalogue/brand/${encodeURIComponent(brandName)}`,
      type: "website",
    },
    alternates: {
      canonical: `${baseUrl}/catalogue/brand/${encodeURIComponent(brandName)}`,
    },
  };
}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {products.map((product) => (
            <Link
              ...
              href={`/catalogue?q=${encodeURIComponent(product.name)}`}
            >
              {/* Image */}
